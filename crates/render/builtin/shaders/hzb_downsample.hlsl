...
void main(uint3 id : SV_DispatchThreadID) {
    if (id.x >= currSize.x || id.y >= currSize.y) return;

    uint2 base = id.xy * 2;

	float r = LoadClamped(base + uint2(0, 0));
	r = min(r, LoadClamped(base + uint2(1, 0)));
	r = min(r, LoadClamped(base + uint2(0, 1)));
	r = min(r, LoadClamped(base + uint2(1, 1)));
