...
use crate::bindings::root::{AKRESULT, AKRESULT::*};
    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(
            ...
            match self {
                ...
                AK_DeviceNotReady =>
                    "specified ID doesn't match a valid hardware device: either the device doesn't \
                ...
                AK_InvalidCustomPlatformName =>
                    "detecting incompatibility between Custom platform of banks and custom \
                     platform of connected application",
                AK_DLLCannotLoad =>
                    "plugin DLL could not be loaded, either because it is not found or one \
                ...
                AK_PluginNotRegistered =>
                    "plugin is not registered.  Make sure to implement a AK::PluginRegistration \
                     class for it and use AK_STATIC_LINK_PLUGIN in the game binary",
                AK_DataAlignmentError =>
                    "a pointer to audio data was not aligned to the platform's required alignment \
                     (check AkTypes.h in the platform-specific folder)",
                ...
                AK_DeviceNotFound =>
                    "the specified device ID does not match with any of the output devices that \
                     the sound engine is currently using",
