...
pub enum MapResource {
    ...
    PushSurface(TagHash, Option<CustomDebugShape>),
    Unk808068d4(TagHash),
...
impl MapResource {
    pub fn debug_string(&self) -> String {
        match self {
            ...
            MapResource::PushSurface(t, s) => {
                if s.is_some() {
                    format!("Push Surface (havok {t})")
                } else {
                    format!(
                        "Push Surface (havok {t})\n{} Havok shape visualization failed to load",
                        ICON_ALERT
                    )
                }
            }
    ...
    pub fn draw_debug_shape(&self, transform: &Transform, debug_shapes: &mut DebugShapes) {
        match self {
            ...
            MapResource::PushSurface(_, Some(shape)) => {
...
mapresource_info!(
    ...
    12, PushSurface, [96, 96, 255], ICON_HELP
