...
impl ComponentPanel for Route {
    ...
    fn show_inspector_ui(
        ...
        if ui
            ...
            .on_hover_text("Traverse Path")
            .clicked()
        {
            if let Some(start_pos) = self.path.first() {
                let mut old_pos = start_pos.pos + Vec3::Z * 0.1;
                let mut old_orient = camera.get_look_angle(start_pos.pos);
                camera.tween = Some(Tween::new(
                    |x| x,
                    Some((camera.position, old_pos)),
                    Some((camera.orientation, camera.get_look_angle(start_pos.pos))),
                    1.0,
                ));
                camera.tween_queue.clear();
                for node in self.path.iter().skip(1) {
                    let new_pos = node.pos + Vec3::Z * 0.1;
                    let new_orient = get_look_angle(old_orient, old_pos, new_pos);

                    camera.tween_queue.push_back(Tween::new(
                        |x| x,
                        None,
